    ...
    for scanner.Scan() { line := scanner.Text()
        if (len(line)>0){
            ...
            if goodentry(name,value) {
                if !(true==p.fixed[name]) { p.raw[name] =  value
                    if invariant { p.fixed[name]=true }
...
func goodentry(key, valstring string)bool{
    if (len(key)>0 && len(valstring)>0){ return true
    } else { return false }
}
...
func trisplit(line string,sep1 string, sep2 string, altsep2 string)(string, string, string,bool){
    ...
    if (len(tokens)>1) {
        ...
        if (len(tokens)>1) { invariant = true }
    } // if not, try alt separator 2
    if (len(tokens)>1) { comment = tokens[1];  } // if split, param cannot be further modified
    ...
    if (len(units)>0) && (len(name)>0) { value = strings.TrimSpace(units[1]);  }
...
func (p *Params) Print(writer io.Writer,blurb string) {
    if blurb != "" { fmt.Fprintln(writer,blurb) }
    fmt.Fprintln(writer,"\n\tinteger parameters")
    for key, value := range p.ints{ fmt.Fprintln(writer,key,"=",value) }
    fmt.Fprintln(writer,"\n\boolean parameters")
    for key, value := range p.bools{ fmt.Fprintln(writer,key,"=",value) }
    fmt.Fprintln(writer,"\n\tstring parameters")
    for key, value := range p.strs{ fmt.Fprintln(writer,key,"=",value) }
    fmt.Fprintln(writer,"\n\tfile parameters")
    for key, value := range p.files{ fmt.Fprintln(writer,key,"=",value) }
    fmt.Fprintln(writer,"\n\tnumber parameters")
    for key, value := range p.nums{ fmt.Fprintln(writer,key,"=",value) }
    fmt.Fprintln(writer,"\n\traw parameters")
    for key, value := range p.raw{ fmt.Fprintln(writer,key,"=",value) }
    fmt.Fprintln(writer,"\n\tfixed (invariant, constant) parameters")
    for key, value := range p.fixed{ if (value) { fmt.Fprintln(writer,key,"is invariant") }}
}
...
func (p *Params) Geti(str string) int {
    if reti, ok := p.ints[str]; ok { return reti
        ...
        return ret
    } else { hardExit(noparam, str) }             // wasn't found anywhere
    return 0
}

func (p *Params) Getn(str string) float64 {
    if retn, ok := p.nums[str]; ok { return retn
    } else if rawstr, ok := p.raw[str]; ok { // check in raw hash
...
func (p *Params) Gets(str string) string {
    if retn, ok := p.strs[str]; ok { return retn
...
func (p *Params) Getf(str string) string {
    fmt.Println("\nGetting file",str,p.raw)
    if retn, ok := p.files[str]; ok { return retn
    } else if retn, ok := p.raw[str]; ok { // check in raw hash
        p.files[str] = retn        // add to files map
        return retn
    } else {
        fmt.Println("\failed file",str,p.raw,p.raw[str])
...
func (p *Params) Getb(str string) bool {
    if retb, ok := p.bools[str]; ok { return retb
    } else if rawstr, ok := p.raw[str]; ok { // check in raw hash
        ret,err := strconv.Atoi(rawstr) // convert to integer
        if err!=nil { fmt.Println(err) }
        var retb bool
        if ret>0 { retb = true } else { retb = false } // only positives are true
        p.bools[str] = retb        // add to ints map
        return retb
    } else { hardExit(noparam, str) }             // wasn't found anywhere
    return false
}
...
func (p *Params) getr(str string) string { // this is lower case because we shouldn't export it'
    if retn, ok := p.raw[str]; ok { return retn
    } else { hardExit(noparam, str) }             // wasn't found anywhere
    return ""
}
...
func SortbyVals (m map[string]int) []string {   p := make(ByValue, len(m));
    i := 0; for k, v := range m { p[i] = Pair{k, v}; i++;  };   // increment i to point to pairs
    sort.Slice(p, func(i, j int) bool { return p[i].Value > p[j].Value })   // sort by Value
    var sortkeys []string                                                       // to hold sorted keys
    for i := range p { sortkeys = append(sortkeys, p[i].Key) }; //  append each Key in order to sortkeys
    return sortkeys
}
